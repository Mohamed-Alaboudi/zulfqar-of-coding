...
| **Trivy** *(optional)* | Auditing a repository's dependency vulnerabilities, IaC misconfigurations, licenses, or SBOM inputs. The native gate handles secrets; use the pinned read-only wrapper rather than adding another always-loaded MCP. | [`scripts/run-trivy.sh`](../../scripts/run-trivy.sh) and [`TRIVY.md`](TRIVY.md) |
