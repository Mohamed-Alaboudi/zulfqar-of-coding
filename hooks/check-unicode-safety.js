...
// Scans text files for dangerous invisible Unicode (the "ASCII smuggling" /
// "Tag smuggling" prompt-injection vector) and stray emoji, and can auto-strip
// them from Markdown with --write. Exits 1 if any violation remains - wire it
// as a pre-commit gate or CI check so hidden instructions can't ride into a
// PR body, SKILL.md, or frontmatter where a human reviewer sees nothing.
//
// Usage:
//   node check-unicode-safety.js            scan (report + exit 1 on violation)
...
// Scan root: CC_UNICODE_SCAN_ROOT if set, else the current working directory.
const repoRoot = process.env.CC_UNICODE_SCAN_ROOT
  ? path.resolve(process.env.CC_UNICODE_SCAN_ROOT)
  : process.cwd();
...
// This scanner necessarily contains the code points it looks for, so never
// auto-rewrite itself or a test fixture that intentionally embeds samples.
const writeModeSkip = new Set([
  path.normalize('hooks/check-unicode-safety.js'),
  path.normalize('scripts/check-unicode-safety.js'),
]);
